        ...
        return {
            ...
            getGeoData: function () {
                ...
                var promise =
                       $http({
                           method: 'GET',
                           url: $rootScope.api + 'GEO/GetGeo?userid='+$localStorage.user.UserID
                       }).then(function (resp) {
                           return resp;
                       });
                return promise;
            },
            getAllGeoData: function () {

                var promise =
                       $http({
                           method: 'GET',
                           url: $rootScope.api + 'GEO/GetAllGeo?userid=' + $localStorage.user.UserID
                       }).then(function (resp) {
                           return resp;
                       });
                return promise;
            },
            getSubRegionData: function () {
            var promise =
                   $http({
                       method: 'GET',
            ...
            getCountriesData: function (param) {
                let countryData={};
                countryData.geoid = param;
                countryData.userid = $localStorage.user.UserID;
                var promise =
                       $http({
                           method: 'GET',
                           url: $rootScope.api + 'GEO/GetUserCountriesByGeo?userid='+$localStorage.user.UserID+'&geoid='+param
                       }).then(function (resp) {
                           return resp;
                       });
                return promise;
            },
            getAllCountriesData: function (param) {
                let countryData = {};
                countryData.geoid = param;
                countryData.userid = $localStorage.user.UserID;
                var promise =
                       $http({
                           method: 'GET',
                           url: $rootScope.api + 'GEO/GetAllUserCountriesByGeo?userid=' + $localStorage.user.UserID + '&geoid=' + param
                       }).then(function (resp) {
                           return resp;
                       });
                return promise;
            ...
            getFinancialYear: function () {
                var data = { FinancialyearID: $localStorage.user.FinancialYearID };
                return $http({
                    method: 'POST',
                    url: $rootScope.api + 'BUBudgets/GetFinancialyearSearch',
                    data: data
            ...
            SC_search: function (searchCriteriaData) {
                return $http({
                    method: 'POST',
                    url: $rootScope.api + 'Search/GetSearchResult',
            ...
            ExportBudgetReport: function () {
                var url = $rootScope.api + "Export/ExporttoExcelBudgetReport";
                window.open(url, '_blank', '');
            },
            ExportCarveoutReport: function () {
                var url = $rootScope.api + "Export/ExporttoExcelCarveoutReport";
                window.open(url, '_blank', '');
            }
    }
    }
